...
pass the acceptance gate (≥5 benefits at 100% agent-backed).
...
| `triggered` | 12 |
| `verified` | 10 |
| `suppressed` | 2 (no chunks above similarity threshold) |
...
managed_scan_session entitlements=12 agent_backed=true mcp_tools=12
```
...
| 4 | `_ingest_event` parsed wrong stop_reason shape | `agent_runner.py` |
| 5 | Tool confirmation used wrong field name | `agent_runner.py` |
| 6 | Scan brief prompt and citation resolver key mismatch | `scan.py` |
| 7 | **Nested managed sessions** — `_verify_local` called `_call_claude` which re-entered the managed-agent path, spawning one full session per entitlement (~57 s each × 12 = 684 s, exceeding the timeout). Fixed by extracting `_call_messages_create` and routing `_verify_local` through it directly. Retry coverage preserved: `_call_messages_create` retries SDK errors; `_call_claude` retries managed-agent transport errors | `verify.py` |
| 8 | `/scan` endpoint did not persist to `sweep.db` — acceptance gate always saw 0 rows | `main.py` |
| 9 | FK violation on `insert_scan` — `users` row must be upserted before `scan_results` insert | `main.py` |
| 10 | `managed_session_timeout_s` 180 s too short for 12 sequential tool calls; raised to 600 s | `config.py` |
...
/scan
  └── _verify_via_managed_session()         ← ONE managed session (outer)
        └── verify_entitlement × 12 (MCP)  ← agent confirms, then executes
...
Total wall-clock: ~204 s for 12 entitlements.
...
| Item | Value |
|---|---|
| `MANAGED_AGENT_ID` | `agent_…op` (version 1, no-slash MCP URLs) |
| `MANAGED_ENVIRONMENT_ID` | `env_…9B` |
| `MANAGED_VAULT_ID` | `vlt_…EN` |
